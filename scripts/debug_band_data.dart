...
void main() async {
  ...
  try {
    ...
    for (final bandDoc in bandsSnapshot.docs) {
      ...
      print(
        'memberUids: ${memberUids ?? 'MISSING'} (${memberUids?.length ?? 0})',
      );
      print('adminUids: ${adminUids ?? 'MISSING'} (${adminUids?.length ?? 0})');
      print(
        'editorUids: ${editorUids ?? 'MISSING'} (${editorUids?.length ?? 0})',
      );
      print('');
      ...
      if (isInMembers) {
        ...
        final memberRole = members.firstWhere(
          (m) => m['uid'] == user.uid,
        )['role'];
      ...
      if (adminUids != null) {
        ...
        if (!adminUidsSet.containsAll(adminMembers)) {
          issues.add(
            'adminUids missing some admins from members: ${adminMembers.toSet().difference(adminUidsSet)}',
          );
        }

        if (!adminMembersSet.containsAll(adminUids.cast<String>())) {
          issues.add(
            'adminUids has extra UIDs not in members: ${adminUidsSet.difference(adminMembersSet)}',
          );
        }
      }
