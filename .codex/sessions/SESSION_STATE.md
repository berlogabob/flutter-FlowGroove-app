...
### Production FTP: Hugo + Flutter Deploy ✅ COMPLETE

#### Changes Made
- **Hugo now deploys to production FTP** — `make deploy-stable` deploys both Hugo and Flutter
- **Architecture parity** — production FTP matches GitHub Pages structure:
  - Hugo landing → `flowgroove.app/` (root)
  - Flutter app → `flowgroove.app/app/` (subdirectory)
- **Makefile updates** — 6 targets changed/added:
  - `hugo-build-prod` — new, builds Hugo with `--baseURL "https://flowgroove.app/"`
  - `build-web-prod` — new, builds Flutter with `--base-href "/app/"`
  - `deploy-stable` — updated pipeline: Hugo → Flutter → backup → FTP → health
  - `ftp-upload` — updated: 2 lftp calls (Hugo → root, Flutter → /app/)
  - `health-check-prod` — updated: checks 3 URLs (Hugo, Flutter, config)
  - `.PHONY` — added new targets
- **Documentation updated** — `README.md`, `ARCHITECTURE.md`, `DEPLOYMENT_GUIDE.md`, `QWEN.md`
